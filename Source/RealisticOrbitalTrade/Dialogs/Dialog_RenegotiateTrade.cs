...
internal class Dialog_RenegotiateTrade : Window
{
    ...
    private void ResolveRenegotiation()
    {
        ...
        /*
                foreach (var originalTradeable in allTradeables)
                {
                    var tradedThingDef = originalTradeable.ThingDef;
                    var newTradeable = tradedThingDef != RimWorld.ThingDefOf.Silver ? cachedTradeables[tradedThingDef] : cachedCurrencyTradeable;

                    if (originalTradeable.CountToTransfer != newTradeable.CountToTransfer)
                    {
                        // This trade was modified

                        if (originalTradeable.CountToTransfer < 0)
                        {
                            // This tradeable was being sold (to the trader)

                            var change = -(newTradeable.CountToTransfer - originalTradeable.CountToTransfer);
                            if (change > 0)
                            {
                                RealisticOrbitalTradeMod.Dev(() => $"Trader gets more {tradedThingDef.label}");

                                if (tradedThingDef != RimWorld.ThingDefOf.Silver)
                                {
                                    RealisticOrbitalTradeMod.Error($"Only expecting to owe the trader silver after renegotiation; but owing {tradedThingDef}?!");
                                    continue;
                                }

                                TransferableUtility.TransferNoSplit(cachedCurrencyTradeable.thingsColony, change, (Thing toGive, int countToGive) =>
                                {
                                    ThingCountClass thingCount = new(toGive, countToGive);
                                    tradeAgreement.thingsSoldToTrader.Add(thingCount);
                                    Utils.AddThingToLoadToShuttle(thingCount, toTraderCompTradeShuttle, toTraderCompShuttle);
                            ...
                            {
                                RealisticOrbitalTradeMod.Dev(() => $"Trader gets less {tradedThingDef.label}");

                                if (originalTradeable.AnyThing is Pawn pawn)
                                {
                                    // Remove from sold list
                                    tradeAgreement.pawnsSoldToTrader.Remove(pawn);
                                    // Remove from shuttle's required cargo list
                                ...
                                {
                                    // We need to take things off the soldToTrader list
                                    var leftToTakeOff = -change;
                                    do
                                    {
                                        ThingCountClass thingCount;
                                        if (originalTradeable.AnyThing.HasRequirements())
                                        {
                                            thingCount = tradeAgreement.thingsSoldToTrader.FirstOrDefault(t => t.thing.GetInnerIfMinified() == originalTradeable.AnyThing);
                                        }
                                        else
                                        {
                                            thingCount = tradeAgreement.thingsSoldToTrader.FirstOrDefault(t => t.thing.def == tradedThingDef);
                                        ...
                                        if (thingCount == null)
                                        {
                                            //RealisticOrbitalTradeMod.Error($"Needed to take {-change} off the 'thingsSoldToTrader' list, but could only remove {-change - leftToTakeOff}");
                                            break;
                                        }
                                        if (thingCount.Count > leftToTakeOff)
                                        {
                                    ...
                                    // We need to take things off the to-be-loaded list
                                    leftToTakeOff = -change;
                                    do
                                    {
                                        // First, remove things from the to-be-loaded list. If we can get away 
                                        // with just reducing that, we won't need to unload anything.
                                        if (originalTradeable.AnyThing.HasRequirements())
                                        {
                                            var thingDefCountMaybeNull = toTraderCompTradeShuttle.requiredSpecificItems.Cast<ThingDefCountWithRequirements?>().FirstOrDefault(t => originalTradeable.ThingDef == t!.Value.def);
                                            if (!thingDefCountMaybeNull.HasValue)
                                            {
                                                RealisticOrbitalTradeMod.Error($"Needed to take {-change} off the 'requiredItems' list, but could only remove {-change - leftToTakeOff}");
                                                break;
                                            }
                                            var thingDefCount = thingDefCountMaybeNull.Value;
                                            if (thingDefCount.count > leftToTakeOff)
                                        ...
                                        {
                                            var thingDefCount = toTraderCompShuttle.requiredItems.FirstOrDefault(t => originalTradeable.ThingDef == t.ThingDef);
                                            if (thingDefCount == null)
                                            {
                                                RealisticOrbitalTradeMod.Error($"Needed to take {-change} off the 'requiredItems' list, but could only remove {-change - leftToTakeOff}");
                                                break;
                                    ...
                                    if (originalTradeable.AnyThing.HasRequirements())
                                    {
                                        actuallyLoaded = toTraderTransportShipContainer.Where(t => t.GetInnerIfMinified() == newTradeable.AnyThing).ToList();
                                        countActuallyLoaded = actuallyLoaded.Sum(t => t.stackCount);
                                    }
                                    else
                                    {
                                        actuallyLoaded = toTraderTransportShipContainer.Where(t => t.GetInnerIfMinified().def == newTradeable.ThingDef).ToList();
                                        countActuallyLoaded = actuallyLoaded.Sum(t => t.stackCount);
                                    }

                                    var countRequiredLoaded = newTradeable.CountToTransferToDestination;
                                    if (countActuallyLoaded > countRequiredLoaded)
                                    {
                                        // More items loaded than we've sold now; unload some
                                        var leftToUnload = countActuallyLoaded - countRequiredLoaded;
                                    ...
                                    {
                                        foreach (var p in PawnsWithHaulToTransporterJob(toTraderCompTransporter, toTraderShipThing.Map)
                                            .Where(jp => jp.carryTracker.CarriedThing == newTradeable.AnyThing || jp.CurJob.targetA.Thing.GetInnerIfMinified() == newTradeable.AnyThing))
                                        {
                                            RealisticOrbitalTradeMod.Dev(() => $"Cancelling hauling job for {p} for {originalTradeable.AnyThing.LabelNoCount}");
                                            p.jobs.EndCurrentJob(JobCondition.Incompletable, false, true);
                                        }
                                    }
                                    else
                                    {
                                        foreach (var p in PawnsWithHaulToTransporterJob(toTraderCompTransporter, toTraderShipThing.Map)
                                            .Where(jp => jp.carryTracker.CarriedThing?.def == newTradeable.ThingDef || jp.CurJob.targetA.Thing.def == newTradeable.ThingDef))
                                        {
                                            RealisticOrbitalTradeMod.Dev(() => $"Cancelling hauling job for {p} for {originalTradeable.AnyThing.LabelNoCount}");
                                            p.jobs.EndCurrentJob(JobCondition.Incompletable, false, true);
                                        }
                            ...
                            RealisticOrbitalTradeMod.Dev(() => $"To trader: {originalTradeable} amount (b/a): {originalTradeable.CountToTransfer},{newTradeable.CountToTransfer};change: {change}");
                        }
                        else
                        {
                            // This tradeable is being bought (goes to the colony)
                            // These are easiest to deal with because they arrive with a separate,
                            // later shuttle, and thus can be directly moved between inventories.
                            var change = newTradeable.CountToTransfer - originalTradeable.CountToTransfer;
                            RealisticOrbitalTradeMod.Dev(() => $"To colony: {originalTradeable} amount (b/a): {originalTradeable.CountToTransfer},{newTradeable.CountToTransfer};change: {change}");
                            if (change > 0)
                            {
                                // The only thing a colony should be able to get more of in a renegotiation is silver
                                if (tradedThingDef != RimWorld.ThingDefOf.Silver)
                                {
                                    RealisticOrbitalTradeMod.Error($"Expected only silver here, got {tradedThingDef}");
                                    continue;
                                }
                                // More of something.
                                RealisticOrbitalTradeMod.Dev(() => $"Colony gets more {tradedThingDef.label}");

                                // Move silver out of the trader and into the transport ship
                                TransferableUtility.TransferNoSplit(cachedCurrencyTradeable.thingsTrader, change, (Thing toGive, int countToGive) =>
                                {
                                    toPlayerTransportShipContainer.TryAddOrTransfer(toGive, countToGive);
                                });
                                int newCount = toPlayerTransportShipContainer.Where(t => t.def == RimWorld.ThingDefOf.Silver).Sum(t => t.stackCount);
                                if (newCount != newTradeable.CountToTransferToSource)
                                {
                                    RealisticOrbitalTradeMod.Warning($"Was only able to make amount of {tradedThingDef.label} to shuttle {newCount} for the player; wanted {newTradeable.CountToTransferToSource}. This is only an error if the player did not get the warning dialog about it.");
                                }
                            }
                            else
                            {
                                // Less of something.
                                RealisticOrbitalTradeMod.Dev(() => $"Colony gets less {tradedThingDef.label}");

                                // Move things out of the transport ship and back to the trader
                                var things = originalTradeable.thingsTrader;
                                foreach (var thing in things)
                                {
                                    var originalStackCount = thing.stackCount;
                                    RealisticOrbitalTradeMod.Dev(() => $"-- Stack count: {originalStackCount}; asking to transfer {-change}");
                                    tradeShipThings.TryAddOrTransfer(thing, -change);
                                    change += originalStackCount;
                                }
                                int newCount = toPlayerTransportShipContainer.Where(t => t.def == tradedThingDef).Sum(t => t.stackCount);
                                if (change != 0)
                                {
                                    if (tradedThingDef != RimWorld.ThingDefOf.Silver)
                                    {
                                        RealisticOrbitalTradeMod.Error($"Only expecting to potentially owe the trader silver after renegotiation; but owing {tradedThingDef}?!");
                                        continue;
                                    }
