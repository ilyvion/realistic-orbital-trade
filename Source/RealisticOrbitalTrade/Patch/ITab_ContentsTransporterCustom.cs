...
{
    private static readonly MethodInfo _method_ITab_ContentsBase_DoThingRow = AccessTools.Method(typeof(ITab_ContentsBase), "DoThingRow");

    private static readonly MethodInfo _method_ITab_ContentsTransporterCustom_DoThingRow = AccessTools.Method(typeof(ITab_ContentsTransporterCustom), nameof(ITab_ContentsTransporterCustom.DoThingRow));

    [HarmonyReversePatch]
    internal static void DoItemsLists(ITab_ContentsTransporter instance, Rect inRect, ref float curY)
    {
        IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            codeMatcher.SearchForward(i => i.opcode == OpCodes.Call && i.operand is MethodInfo m && m == _method_ITab_ContentsBase_DoThingRow);
            if (!codeMatcher.IsValid)
            {
                RealisticOrbitalTradeMod.Error("Could not reverse patch ITab_ContentsTransporter.DoItemsLists, IL does not match expectations: call to ITab_ContentsBase:DoThingRow not found.");
...
#pragma warning restore CS8625
    ...
    [HarmonyReversePatch]
    internal static void DoThingRow(ITab_ContentsBase instance, ThingDef thingDef, int count, List<Thing> things, float width, ref float curY, Action<int> discardAction)
    {
        IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            var originalInstructionList = instructions.ToList();
            ...
            if (!codeMatcher.IsValid)
            {
                RealisticOrbitalTradeMod.Error("Could not reverse patch ITab_ContentsBase.DoThingRow, IL does not match expectations: call to Widgets::InfoCardButton(Def) not found.");
                return originalInstructionList;
            }
            ...
            if (!codeMatcher.IsValid)
            {
                RealisticOrbitalTradeMod.Error("Could not reverse patch ITab_ContentsBase.DoThingRow, IL does not match expectations: call to Widgets::ThingIcon(Def) not found.");
                return originalInstructionList;
            }
            ...
            if (!codeMatcher.IsValid)
            {
                RealisticOrbitalTradeMod.Error("Could not reverse patch ITab_ContentsBase.DoThingRow, IL does not match expectations: call to set value of Text::WordWrap not found.");
                return originalInstructionList;
            }
